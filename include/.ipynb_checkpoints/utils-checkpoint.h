...
//     }
// }
static void VisualizeMatching(const cv::Mat &image0, const std::vector<cv::KeyPoint> &keypoints0, 
                              const cv::Mat &image1, const std::vector<cv::KeyPoint> &keypoints1,
                              const std::vector<cv::DMatch> &superglue_matches, cv::Mat &output_image, 
                              double cost_time = -1) {
    if (image0.size != image1.size) return;

    // 画匹配点
    cv::drawMatches(image0, keypoints0, image1, keypoints1, superglue_matches, output_image, 
                    cv::Scalar(0, 255, 0), cv::Scalar(0, 0, 255));

    
    // 计算自适应缩放因子
    double sc = std::max(std::min(image0.rows / 640.0, 2.0), 0.5);
    double font_scale = std::max(0.5, 1.2 * sc);  
    int ht = std::max(20, static_cast<int>(42 * sc));  
    int thickness = std::max(1, static_cast<int>(2 * sc));  
